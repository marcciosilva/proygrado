...
## relu:
name = "relu_2"
all_data = get_data(paste(name,".csv",sep=""),"ANN - 2 capas ocultas. Activación: relu.")
...
subtitle = "Porcentaje de diferencia entre makespan obtenido y esperado en clasificación. \nSVM y ANN 2 capas ocultas con activación relu."
...
ggsave("2_medianas_diferenciasann_2_capas_ocultas_relu.png",width = 25, height = 15, units = "cm",dpi = 300)
...
p <- ggplot(all_data) + 
  geom_boxplot(outlier.shape= NA, data=all_data,aes(x=all_data$rango, y=Y,fill=Clasificador),position=position_dodge(width = 0.9)) +
  scale_fill_grey(start = 0.4, end = 1) +
  coord_cartesian(ylim=c(0.80,0.5)) +
  labs(x=x_title,y=y_title) +
  theme(plot.title = element_text(family = "Roboto", color="#666666", face="bold", size=11, hjust=0)) +
  theme(axis.title = element_text(family = "Roboto", color="#666666", face="bold", size=9)) +
  theme(plot.subtitle = element_text(family = "Roboto", color="#666666", size=11)) +
  theme(legend.position="bottom")
ggsave("3_accuracy_ann_2_capas_ocultas_relu.png",width = 25, height = 15, units = "cm",dpi = 300)


title = "Comparación entre Red Neuronal y Máquina de Soporte Vectorial."
x_title = "Rango de tareas en dimensiones estudiadas."
y_title = "Porecentaje de selecciones de mejores máquinas cuando \nse selecciona una máquina errona"
subtitle = "Precisión en clasificación. \nSVM y ANN 2 capas ocultas con activación relu."
...
p <- ggplot(all_data) + 
  ...
  theme(plot.subtitle = element_text(family = "Roboto", color="#666666", size=11)) +
  theme(legend.position="bottom")
ggsave("4_porcentaje_maquinas_mejores_ann_2_capas_ocultas_relu.png",width = 25, height = 15, units = "cm",dpi = 300)

'''
title = "Comparación entre Red Neuronal y Máquina de Soporte Vectorial."
x_title = "Rango de tareas en dimensiones estudiadas."
y_title = "Error relativo del tiempo total de ejecución agregado por la selección de máquinas erroneas\n(obtenido - esperado)/esperado"
subtitle = "Error relativo. \nSVM y ANN 2 capas ocultas con activación relu."
Y = all_data$error_relativo
p <- ggplot(all_data) + 
  geom_boxplot(outlier.shape= NA, data=all_data,aes(x=all_data$rango, y=Y,fill=Clasificador),position=position_dodge(width = 0.9)) +
  scale_fill_grey(start = 0.4, end = 1) +
  ...
  theme(plot.subtitle = element_text(family = "Roboto", color="#666666", size=11)) +
  theme(legend.position="bottom")
ggsave("6_error_relativo_ann_2_capas_ocultas_relu.png",width = 25, height = 15, units = "cm",dpi = 300)
